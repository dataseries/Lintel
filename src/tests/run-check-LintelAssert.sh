...
) 2>&1 | grep -v './run-check-LintelAssert.sh' | grep -v 'Aborted$' | grep -v '^Aborted' | sed "s/.cpp, [0-9]*:/.cpp, LINENO:/" | sed 's,Assert failure in .*/tests/check-Lintel,Assert failure in tests/check-Lintel,' >./tmp-LintelAssert.out
set -e
