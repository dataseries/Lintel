...
int main() {    	
	...
	cout << "\n Range comparison tests begin \n";

	for (uint32_t i = EqualRangeComparator; i <= FirstRangeNull; i++) {
		
		prepareVectors((rangeComparator)i, retValue, comparisonType);
              INVARIANT(retValue == iteratorRangeEqual(first_range.begin(), first_range.end(), second_range.begin(), second_range.end()), format("Comparison type = %s") % comparisonType);		
		clearRanges();
    	}    
	cout << "\n Range comparison tests completed \n";
