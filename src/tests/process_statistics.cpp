...
int main() {
    ...
    SINVARIANT(abs_diff(initial_resident - once_resident) < 100*page_size); 
    size_t once_resident_pid = ProcessStatistics::getOnce(ResidentSize, getpid());
    INVARIANT(abs_diff(initial_resident - once_resident_pid) < 100*page_size,
              format("%d %d %.0f\n") % initial_resident % once_resident_pid
              % abs_diff(initial_resident - once_resident_pid));

    size_t initial_size = proc_stats.getCached(AddressSize);
    SINVARIANT(abs_diff(initial_size - ProcessStatistics::getOnce(AddressSize)) < 100*page_size);
    SINVARIANT(abs_diff(initial_size - ProcessStatistics::getOnce(AddressSize, getpid())) 
               < 100*page_size);
    ...
    if (deptool_info.osVersion() == "opensuse-12.1") {
        ...
        INVARIANT(abs_diff(proc_stats.getCached(ResidentSize) - initial_resident) < 100*page_size,
                  format("failed on %s") % deptool_info.osVersion()); 
    }
    
    // But size should have changed    
    SINVARIANT(abs_diff(proc_stats.getCached(AddressSize) - initial_size) > 9000*page_size);
    ...
    SINVARIANT(abs_diff(proc_stats.get(ResidentSize) - initial_resident) > 9000 * page_size);     
}
