...
/** @file
    ...
    have a function (OptionalCubePartialFn) that can decide on a
...
namespace lintel {
	...
	inline bool cubeHadFalse(bool had_false) {
	    return had_false;
	}
	...
	typedef boost::function<void (const Tuple &key, 
				      typename MyAny::AnyT, StatsT *value)> WalkFn;

	typedef boost::function<bool (bool had_false, const MyAny &)> OptionalCubePartialFn;
	...
	explicit 
	StatsCube(const OptionalCubePartialFn &fn1 = boost::bind(&StatsCubeFns::cubeHadFalse, _1),
	...
	/// @param hts.  This function will not create entries in the
	...
	void cube(const HashTupleStats<Tuple, StatsT> &hts) {
	    hts.walk(boost::bind(&StatsCube<Tuple, StatsT>::cubeAddOne, this, _1, _2));
	}
	...
	/// a very big cube.
	void zeroCube(const HashTupleStats<Tuple, StatsT> &hts) {
	    HashUniqueTuple hut;
	    hts.fillHashUniqueTuple(hut);
	    zeroCube(hts, hut);
	}

	/// Cube the data in @param hts, including all zero values
	/// implied by @param hut.  This version of the function lets
	/// you prune out parts of the space that you don't want to
	/// cube over by removing values from hut.  For example, if
	/// you clear out one of the hut tuple entries, it is the
	/// equivalent of requiring that value to be any.
    ...
    private:
	void cubeAddOne(MyAny &key, size_t pos, bool had_false, const StatsT &value) {
	    if (pos == key.length) {
